...
function Homepage() {
  ...
  useEffect(() => {
    ...
    if (username) {
      ...
      fetch(`http://localhost:8000/api/users/${username}/friend-activity`)
        .then((res) => (res.ok ? res.json() : []))
            ...
            {friendActivity.map((item) => (
              <div key={item.id} style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "15px", paddingBottom: "15px", borderBottom: "1px solid #333" }}>
                ...
                <div style={{ fontSize: "0.9rem", lineHeight: "1.4" }}>
                  ...
                  <span onClick={() => item.link && navigate(item.link)} style={{ fontWeight: "bold", color: "#fff", cursor: item.link ? "pointer" : "default", textDecoration: item.link ? "underline" : "none" }}>{item.target}</span>
