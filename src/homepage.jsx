...
function Homepage() {
  ...
  const [friendRecommendedAlbums, setFriendRecommendedAlbums] = useState([]); // NEW STATE
  ...
  useEffect(() => {
    ...
    if (username) {
      ...
      // 2. Fetch Friend Recommendations (NEW FETCH)
      fetch(`http://localhost:8000/api/albums/friends-recommended?username=${username}`)
        .then((res) => (res.ok ? res.json() : []))
        .then((data) => setFriendRecommendedAlbums(data))
        .catch((err) => console.error(err));
    }

    // 3. Fetch Recommended
    const recommendedUrl = username
      ? `http://localhost:8000/api/albums/recommended?username=${username}`
      : "http://localhost:8000/api/albums/recommended";
    ...
    <div style={{ padding: "30px", maxWidth: "1600px", margin: "0 auto", color: "white" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "30px", position: "relative" }}>
        ...
        <div style={{ position: "relative" }}>
          <div onClick={toggleDropdown} style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer", background: "#222", padding: "5px 15px 5px 5px", borderRadius: "30px", border: "1px solid #333" }}>
            {currentUser ? (
            ...
            <span style={{ fontWeight: "bold", fontSize: "0.9rem" }}>
              {currentUser ? currentUser.username : "Login/SignUp"} {currentUser && "▼"}
            </span>
          </div>

          {showProfileMenu && currentUser && (
            <div style={{ position: "absolute", top: "55px", right: 0, backgroundColor: "#222", border: "1px solid #444", borderRadius: "8px", width: "150px", boxShadow: "0 5px 15px rgba(0,0,0,0.5)", zIndex: 50, overflow: "hidden" }}>
              <div onClick={() => navigate(`/profile/${currentUser.username}`)} style={{ padding: "12px 15px", cursor: "pointer", borderBottom: "1px solid #333", fontSize: "0.9rem", color: "white" }} onMouseEnter={(e) => (e.target.style.background = "#333")} onMouseLeave={(e) => (e.target.style.background = "transparent")}>Profile</div>
              <div onClick={handleDropdownLogout} style={{ padding: "12px 15px", cursor: "pointer", fontSize: "0.9rem", color: "#ff5555" }} onMouseEnter={(e) => (e.target.style.background = "#333")} onMouseLeave={(e) => (e.target.style.background = "transparent")}>Logout</div>
            </div>
          )}
        </div>
      </header>
          ...
          <section style={{ marginBottom: "40px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "20px" }}>
              ...
              {recommendedAlbums.map((album) => (
                <div key={album.id} onClick={() => navigate(`/album/${album.id}`)} style={{ cursor: "pointer", backgroundColor: "#181818", padding: "15px", borderRadius: "8px", transition: "background 0.2s", position: "relative" }}>

{/* ONLY show badge if it is based on mood. Hides "Made For You" and "Trending" labels */}
{album.reason && album.reason.includes("feel") && (
  <div style={{ 
    position: "absolute", 
    top: "22px", 
    right: "22px", 
    ...
    {album.reason}
          ...
          {/* 2. FRIEND RECOMMENDATIONS SECTION (NEW) */}
          {friendRecommendedAlbums.length > 0 && (
            <section style={{ marginBottom: "40px" }}>
              <h2 style={{ marginBottom: "20px", color: "#065fd4" }}>Because Your Friends Like It</h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "20px" }}>
                {friendRecommendedAlbums.map((album) => (
        ...
        <aside style={{ flex: 1, minWidth: "250px", maxWidth: "350px" }}>
          ...
          <div style={{ backgroundColor: "#1a1a1a", padding: "20px", borderRadius: "12px", border: "1px solid #333" }}>
            {friendActivity.map((item) => (
              <div key={item.id} style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "15px", paddingBottom: "15px", borderBottom: "1px solid #333" }}>
                ...
                <div style={{ fontSize: "0.9rem", lineHeight: "1.4" }}>
                  <span onClick={() => navigate(`/profile/${item.user}`)} style={{ fontWeight: "bold", color: "#fff", cursor: "pointer", textDecoration: "underline" }}>{item.user}</span>
            ...
            <div onClick={() => navigate("/scenes")} style={{ textAlign: "center", color: "#065fd4", fontSize: "0.9rem", cursor: "pointer", marginTop: "10px" }}>
              Find Friends
            </div>
          </div>
        </aside>
