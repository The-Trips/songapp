...
function AlbumPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const reviewRes = await fetch(`http://localhost:8000/api/albums/${id}/reviews`);
  ...
  const handleMoodClick = async (moodId) => {
    if (!currentUsername) {
        if(window.confirm("Log in to vote on the album's vibe?")) navigate('/login');
        return;
    }
    // Optimistic Update
    setMoods(prev => prev.map(m => m.id === moodId ? { ...m, selected: !m.selected, count: m.selected ? m.count - 1 : m.count + 1 } : m));

    try {
        await fetch('http://localhost:8000/api/moods/toggle', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ album_id: id, mood_id: moodId, username: currentUsername })
        });
        fetchMoods();
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      const payload = { album_id: id, rating: userRating, text: reviewText, username: currentUsername };
      const res = await fetch('http://localhost:8000/api/reviews', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload)
      });
      if (res.ok) {
        const newReview = { user: currentUsername, text: reviewText, rating: userRating, date: new Date().toISOString().split('T')[0] };
  ...
  if (loading || !albumData) return <div style={{padding:'50px', color:'white'}}>Loading...</div>;
  ...
  return (
    <div style={styles.pageContainer}>
      ...
      <div style={styles.heroSection}>
        ...
        <div style={styles.infoContainer}>
            ...
            <div style={styles.actionRow}>
                ...
                <button onClick={handleCommunityClick} style={styles.secondaryButton}>
                    👥 Community
                </button>
            </div>
        </div>
      ...
      <div style={styles.section}>
        ...
        <p style={{color:'#888', marginBottom:'15px', fontSize:'0.9rem'}}>How does this album make you feel?</p>
        <div style={styles.moodGrid}>
            {moods.slice(0, 15).map(mood => (
                ...
                >
                    {mood.name} 
                    <span style={{ opacity: 0.6, fontSize: '0.8em', marginLeft: '8px', background: mood.selected ? 'rgba(0,0,0,0.2)' : 'rgba(255,255,255,0.1)', padding: '2px 6px', borderRadius: '10px' }}>
                        {mood.count}
                    </span>
                </button>
            ))}
      ...
      <div style={styles.section}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:'20px'}}>
            ...
            <span style={{color: '#aaa'}}>Avg Rating: <b style={{color:'white'}}>{averageRating}</b> / 5</span>
        </div>
        <div style={styles.reviewGrid}>
            {reviews.length === 0 ? <p style={{color:'#666'}}>No reviews yet.</p> : reviews.map((r, i) => (
                <div key={i} style={styles.reviewCard}>
                    <div style={styles.reviewHeader}>
                        <div style={{fontWeight:'bold', color:'#fff'}}>{r.user}</div>
                        <div style={{color: '#f5c518'}}>{'★'.repeat(Math.round(r.rating || 0))}<span style={{color:'#444'}}>{'★'.repeat(5 - Math.round(r.rating || 0))}</span></div>
                    </div>
        ...
        <div style={styles.modalOverlay}>
            <div style={styles.modalContent}>
                ...
                <div style={styles.starContainer}>
                    {[1, 2, 3, 4, 5].map((star) => (
                        <span key={star} style={{ fontSize: '2.5rem', cursor: 'pointer', color: (hoverRating || userRating) >= star ? '#f5c518' : '#444', transition: 'color 0.2s' }}
                            onMouseEnter={() => setHoverRating(star)} onMouseLeave={() => setHoverRating(0)} onClick={() => setUserRating(star)}>★</span>
                    ))}
                </div>
                <textarea placeholder="Write your review here..." value={reviewText} onChange={(e) => setReviewText(e.target.value)} style={styles.textArea} />
                <div style={styles.modalActions}>
                    <button onClick={() => setShowReviewModal(false)} style={styles.cancelButton}>Cancel</button>
                    <button onClick={handleSubmitReview} style={styles.submitButton} disabled={isSubmitting}>{isSubmitting ? 'Saving...' : 'Post Review'}</button>
                </div>
            </div>
        </div>
      )}
...
const styles = {
    ...
    coverContainer: { width: '260px', height: '260px', borderRadius: '8px', overflow: 'hidden', boxShadow: '0 8px 30px rgba(0,0,0,0.5)' },
    coverImage: { width: '100%', height: '100%', objectFit: 'cover' },
    infoContainer: { flex: 1, minWidth: '300px' },
    subHeader: { fontSize: '0.85rem', fontWeight: 'bold', letterSpacing: '2px', color: '#1db954', marginBottom: '10px' },
    title: { fontSize: '3.5rem', fontWeight: '900', margin: '0 0 15px 0', lineHeight: '1.1' },
    metaRow: { display: 'flex', alignItems: 'center', gap: '8px', color: '#ccc', fontSize: '1rem', fontWeight: '500', marginBottom: '15px' },
    artistName: { color: 'white', fontWeight: 'bold' },
    dot: { fontSize: '1.5rem', lineHeight: '0' },
    actionRow: { display: 'flex', gap: '15px' },
    primaryButton: { padding: '14px 32px', borderRadius: '30px', border: 'none', backgroundColor: '#1db954', color: 'black', fontSize: '1rem', fontWeight: 'bold', cursor: 'pointer', display:'flex', alignItems:'center', gap:'8px' },
    ...
    moodChip: { padding: '8px 16px', borderRadius: '20px', border: '1px solid #444', cursor: 'pointer', fontSize: '0.9rem', display: 'flex', alignItems: 'center', transition: 'all 0.2s ease' },
    reviewGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' },
    reviewCard: { backgroundColor: '#181818', padding: '20px', borderRadius: '8px', border: '1px solid #282828' },
    ...
    textArea: { width: '100%', height: '100px', backgroundColor: '#333', border: 'none', borderRadius: '8px', padding: '15px', color: 'white', fontSize: '1rem', marginBottom: '20px', resize: 'none' },
