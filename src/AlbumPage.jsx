// src/AlbumPage.jsx updated to use scene database to connect artist album to community
...
function AlbumPage() {
  ...
  const [sceneId, setSceneId] = useState(null); // Track if album has an official community
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/albums/${id}`)
      ...
      .then(data => {
        setAlbumData(data);
        // Check if album has a scene_id (official community)
        if (data.scene_id) {
    ...
    fetch(`${API_URL}/api/albums/${id}/reviews`)
      .then(res => res.json())
    ...
    <div style={{ padding: '40px', color: 'white', maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: '40px', alignItems: 'flex-end', marginBottom: '30px' }}>
        ...
        <div>
          ...
          <p style={{ maxWidth: '600px', marginTop: '20px', color: '#bbb', lineHeight: '1.6' }}>
            {albumData.description || "No description available."}
          </p>

          {/* Official Community Button - Only shows if album has a scene_id */}
          {sceneId && (
            <button
              onClick={() => navigate(`/community/${sceneId}`)}
              style={{
                marginTop: '20px',
                padding: '12px 24px',
                borderRadius: '25px',
          ...
          <div style={{ backgroundColor: '#1a1a1a', padding: '25px', borderRadius: '12px', border: '1px solid #333' }}>
            <div style={{ marginBottom: '20px' }}>
              <label style={{ display: 'block', marginBottom: '10px', fontWeight: 'bold' }}>
                Rating: <span style={{ color: '#1db954', fontSize: '1.2rem' }}>{rating}/10</span>
              </label>
              <input
                type="range"
                min="1"
