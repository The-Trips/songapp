...
const RenderReplies = ({ replies, depth = 0, replyingTo, setReplyingTo, replyInputs, handleReplyInputChange, handlePostReply, handleCancelReply, username, maxDepth = 3 }) => {
    ...
    <div className="replies-container">
      {replies.map((reply) => (
        <div key={reply.id} className="reply-item">
          ...
          {depth < maxDepth && (
            <button className="reply-btn" onClick={() => setReplyingTo(replyingTo === reply.id ? null : reply.id)}>
              💬 Reply
            </button>
          )}
          ...
          {replyingTo === reply.id && (
            <div className="reply-input-container">
              <textarea
                ...
                className="reply-textarea" rows={3}
              />
              <div className="reply-actions">
                <button onClick={() => handlePostReply(reply.id, depth)} className="post-reply-btn">Post</button>
                <button onClick={() => handleCancelReply(reply.id)} className="cancel-btn">Cancel</button>
              </div>
            </div>
          )}
          <RenderReplies replies={reply.replies || []} depth={depth + 1} replyingTo={replyingTo} setReplyingTo={setReplyingTo} replyInputs={replyInputs} handleReplyInputChange={handleReplyInputChange} handlePostReply={handlePostReply} handleCancelReply={handleCancelReply} username={username} maxDepth={maxDepth} />
        </div>
      ))}
    </div>
  );
...
function DiscussionThread({ isAuthenticated }) {
  ...
  const saveToLocalStorage = (updatedComments) => {
    localStorage.setItem(`discussion_${discussionId}`, JSON.stringify({ discussion, comments: updatedComments }));
  };
  ...
  const addReplyToComment = (comments, targetId, newReply) => {
    return comments.map(comment => {
      if (comment.id === targetId) return { ...comment, replies: [...(comment.replies || []), newReply] };
      else if (comment.replies) return { ...comment, replies: addReplyToComment(comment.replies, targetId, newReply) };
      return comment;
    });
  };
  ...
  if (!discussion) return <div style={{padding:'40px', color:'white'}}>Loading...</div>;

  return (
    <div style={{ padding: '40px', color: 'white', maxWidth: '1000px', margin: '0 auto' }}>
      <button onClick={() => navigate(`/album/${id}/discussions`)} className="back-button">← Back to Discussions</button>
      
      <div className="thread-header">
        <h1>{discussion.title}</h1>
        <div className="thread-meta">
          <span className="author-name">{discussion.author}</span> • <span className="time">{discussion.createdAt}</span>
        </div>
      </div>

      <div className="original-post"><p>{discussion.content}</p></div>
        ...
        <div className="comments-list">
          {comments.map((comment) => (
            <div key={comment.id} className="comment-item">
              ...
              <RenderReplies replies={comment.replies} depth={1} replyingTo={replyingTo} setReplyingTo={setReplyingTo} replyInputs={replyInputs} handleReplyInputChange={(id, val) => setReplyInputs({...replyInputs, [id]: val})} handlePostReply={handlePostReply} handleCancelReply={() => setReplyingTo(null)} username={username} />
            </div>
          ))}
        </div>
      </div>
